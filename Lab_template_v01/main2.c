...
enum StatusCode PICC_REQA_or_WUPA(uint8_t command,uint8_t *bufferATQA,uint8_t *bufferSize) {
	...
	PRINTF("clear returned\n\r");
	validBits = 7;									// For REQA and WUPA we need the short frame format - transmit only 7 bits of the last (and only) byte. TxLastBits = BitFramingReg[2..0]
	status = PCD_TransceiveData(&command, 1, bufferATQA, bufferSize, &validBits,&rxAlign,crc);
	PRINTF("trancived returned\n\r");
	if (status != STATUS_OK) {
		PRINTF("\n\r");
		return status;
	}
...
enum StatusCode PICC_RequestA(	uint8_t *bufferATQA,uint8_t *bufferSize) {
	enum StatusCode status;
	PRINTF("request A\n\r");
	status = PICC_REQA_or_WUPA(PICC_CMD_REQA, bufferATQA, bufferSize);
	PRINTF("req returned A\r\n");
	return status;
} // End PICC_RequestA()
...
void PCD_ReadRegister(uint8_t reg,uint8_t value) {
	uint32_t j;
